...
password varchar(30),
primary key(id),
constraint FK_EMPLOYEE_POSITION foreign key(idPosition) references POSITIONM(id)
);
...
primary key(id),
constraint FK_DETAILPRODUCT_PRODUCT foreign key(idProduct) references PRODUCT(id),
constraint FK_DETAILPRODUCT_SIZE foreign key(idSize) references SIZE(id),
constraint FK_DETAILPRODUCT_COLOR foreign key(idColor) references COLOR(id)
);
...
constraint FK_DETAILINVOICE_INVOICE foreign key(idInvoice) references INVOICE(id),
constraint FK_DETAILINVOICE_DETAILPRODUCT foreign key(idDetaiProduct) references DETAILPRODUCT(id)
);